...
// Pega o value de cada input e adiciona em um objeto usando o name de chave
// É pra pegar os valores de formulário mais fácil
function getValoresInput(elemento) {
    ...
    $container.find('input[name]').each(function () {
        ...
        let type = ($input.attr('type') || 'text').toLowerCase();
    ...
    // Gambiarra pra funcionar com o select customizado que eu fiz correndo
    $container.find('.select-arca[data-select]').each(function () {
        let selectId = $(this).data('select');
        let $selectNativo = $('#' + selectId);
...
// Pega a sessão logada no localStorage e carrega os dados do usuário na sidebar
...
// Protege a rota dos mal carater >:(
// Impede que algum intruso cidadão consiga acessar telas de adm, por exemplo
...
// Funções pra forms (modal geralmente)
function validarFormulario(bloco) {
    const inputs = getValoresInput($(bloco));
    ...
    for (const [campo, valor] of Object.entries(inputs)) {
        if (valor == "" || !valor) {
            $(`[name="${campo}"]`).closest('.form-group-arca').addClass('error');
            formValido = false;
        }
    }
